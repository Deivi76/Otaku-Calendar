...
export async function fetchSeasonAnime(year, season) {
    try {
        const res = await rateLimitedFetch(`${KITSU_API}/anime?filter[seasonYear]=${year}&filter[season]=${season}&sort=-userCount&page[limit]=20`);
...
export async function fetchTrendingAnime() {
    try {
        const res = await rateLimitedFetch(`${KITSU_API}/anime?sort=-userCount&page[limit]=20`);
        return res.data?.map((anime) => ({
            title: anime.attributes.titles.en_jp || anime.attributes.titles.en || anime.attributes.canonicalTitle,
...
export async function fetchManga() {
    try {
        const res = await rateLimitedFetch(`${KITSU_API}/manga?sort=-userCount&page[limit]=20`);
...
export async function crawlKitsu() {
    const now = new Date();
