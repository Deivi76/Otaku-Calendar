...
export async function searchManga(title) {
    try {
        const res = await rateLimitedFetch(`${MANGADEX_API}/manga?title=${encodeURIComponent(title)}&limit=25&includes[]=cover_art`);
...
export async function fetchLatestManga() {
    try {
        const res = await rateLimitedFetch(`${MANGADEX_API}/manga?order[latestUploadedChapter]=desc&limit=50&includes[]=cover_art`);
        return res.data?.map((manga) => {
            const cover = manga.relationships.find((r) => r.type === 'cover_art');
            const coverUrl = cover?.attributes?.fileName
                ? `https://uploads.mangadex.org/covers/${manga.id}/${cover.attributes.fileName}.256.jpg`
...
export async function fetchChapters(mangaId) {
    try {
        const res = await rateLimitedFetch(`${MANGADEX_API}/manga/${mangaId}/feed?limit=25&includes[]=scanlation_group`);
        return res.data?.map((chapter) => ({
            title: `Chapter ${chapter.attributes.chapter}`,
            content: chapter.attributes.title || `Scan: ${chapter.relationships.find((r) => r.type === 'scanlation_group')?.attributes?.name || 'Unknown'}`,
            source: MANGADEX_API,
            sourceType: 'api',
...
export async function crawlMangaDex() {
