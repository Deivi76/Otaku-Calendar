...
async function fetchFacebookGroup2(sources) {
    ...
    for (const source of sources) {
        try {
            const response = await fetch(`https://graph.facebook.com/v18.0/${source.name}/posts?access_token=${accessToken}&limit=10`, { signal: AbortSignal.timeout(10000) });
            if (!response.ok)
                continue;
            const data = await response.json();
            if (data.data) {
...
async function fetchTwitchGroup2(sources) {
    ...
    let accessToken = '';
    try {
        const tokenResponse = await fetch(`https://id.twitch.tv/oauth2/token?client_id=${clientId}&client_secret=${clientSecret}&grant_type=client_credentials`, { method: 'POST', signal: AbortSignal.timeout(10000) });
        if (!tokenResponse.ok)
...
async function fetchTelegramGroup2(sources) {
    ...
    for (const source of sources) {
        ...
        try {
            const response = await fetch(`https://api.telegram.org/bot${botToken}/getUpdates?chat_id=${chatId}&limit=10`, { signal: AbortSignal.timeout(10000) });
            if (!response.ok)
                continue;
            const data = await response.json();
            if (data.result) {
