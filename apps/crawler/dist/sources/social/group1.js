...
async function fetchTwitterGroup1(sources) {
    ...
    for (const source of sources) {
        ...
        try {
            const response = await fetch(`https://api.twitter.com/2/users/by/username/${handle}/tweets?max_results=10`, {
                headers: {
...
async function fetchRedditGroup1(sources) {
    ...
    for (const source of sources) {
        try {
            const response = await fetch(`https://www.reddit.com/r/${source.name}/hot.json?limit=25`, {
                headers: {
...
async function fetchYouTubeGroup1(sources) {
    ...
    for (const source of sources) {
        ...
        try {
            const searchResponse = await fetch(`https://www.googleapis.com/youtube/v3/search?key=${apiKey}&q=anime&channelId=${channelId}&order=date&maxResults=10&part=snippet`, { signal: AbortSignal.timeout(10000) });
