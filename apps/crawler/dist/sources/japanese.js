...
async function fetchNHK(apiUrl) {
    try {
        ...
        if (!res.ok)
            throw new Error(`NHK API error: ${res.status}`);
        const data = await res.json();
        return (data?.list?.all?.[0]?.item || []).map((item) => ({
            title: item.title,
...
async function fetchShoboiCalendar(apiUrl) {
    try {
        ...
        if (!res.ok)
            throw new Error(`Shoboi API error: ${res.status}`);
        const data = await res.json();
        const items = [];
        if (data?.Items) {
            for (const [tid, prog] of Object.entries(data.Items)) {
                items.push({
                    title: prog.Title,
...
async function fetchShangrila(apiUrl) {
    try {
        ...
        if (!res.ok)
            throw new Error(`Shangrila API error: ${res.status}`);
        const data = await res.json();
        return (data.works || []).map((work) => ({
            title: work.title,
            content: work.title_ja || work.title,
