...
async function tmbdDiscoverMovies(page = 1): Promise<CrawledItem[]> {
  if (!TMDB_KEY) return [];
...
async function tmdbSearchAnimeMovies(query: string, page = 1): Promise<CrawledItem[]> {
  if (!TMDB_KEY) return [];
...
async function fetchOMDb(title: string): Promise<CrawledItem[]> {
  if (!OMDB_KEY) return [];
...
async function fetchJikanMovies(): Promise<CrawledItem[]> {
  try {
...
async function fetchAniListMovies(): Promise<CrawledItem[]> {
  try {
...
async function fetchKitsuMovies(): Promise<CrawledItem[]> {
  try {
...
async function fetchAnimeThemesMovies(): Promise<CrawledItem[]> {
  try {
...
async function fetchTraktMovies(): Promise<CrawledItem[]> {
  if (!TRAKT_KEY) return [];
  ...
  try {
    ...
    const animeMovies = (data || []).filter((movie: any) => 
      movie.genres?.some((g: string) => ['anime', 'animation', 'japanese'].includes(g.toLowerCase()))
    );
    
    return animeMovies.slice(0, 50).map((movie: any) => ({
      title: movie.title,
...
async function fetchLetterboxdRss(): Promise<CrawledItem[]> {
  const feeds = [
  ...
  try {
    const results = await Promise.all(
      feeds.slice(0, 1).map(async (feedUrl) => {
        try {
...
async function fetchAnnictRss(): Promise<CrawledItem[]> {
  try {
    const url = 'https://annict.com/works?locale=en';
    const res = await fetchWithTimeout(url);
    
    if (!res.ok) throw new Error(`Annict error: ${res.status}`);
    ...
    return titleMatch.slice(0, 20).map((title: string) => ({
...
async function fetchJapaneseCinema(): Promise<CrawledItem[]> {
  try {
    const res = await fetchWithTimeout(JAPANESE_CINEMA_RSS[0].url);
    
    if (!res.ok) throw new Error(`Japanese cinema error: ${res.status}`);
    ...
    return titleMatch.slice(0, 15).map((title: string) => ({
      title: title.replace(/<[^>]+>/g, ''),
...
export async function crawlGhibliFilms(): Promise<CrawledItem[]> {
  ...
  const ghibliFiltered = results.flat().filter(item => 
    item.title.toLowerCase().includes('ghibli') || 
...
export async function crawlAnimeFilms2024(): Promise<CrawledItem[]> {
  const year = 2024;
  const queries = [
    `${year} anime movie`,
    `anime film ${year}`,
  ...
  const filtered = [...tmbd, ...omdb, ...anilist].filter(item => 
    item.publishedAt?.startsWith(String(year)) || 
