 ...
 */
export function calculateGenreMatch(animeGenres, preferredGenres) {
    if (!animeGenres.length || !preferredGenres.length)
        return 0;
 ...
 */
export function calculateRatingScore(animeGenres, highRatedAnimes) {
    if (!highRatedAnimes.length)
        return 0;
    let totalScore = 0;
    let count = 0;
    for (const rated of highRatedAnimes) {
        const ratedGenres = parseGenres(rated.genres);
        const genreOverlap = animeGenres.filter(g => ratedGenres.some(rg => rg.toLowerCase() === g.toLowerCase())).length;
        if (genreOverlap > 0) {
 ...
 */
export function calculatePopularityScore(animeScore) {
    if (!animeScore)
        return 0;
 ...
 */
export function calculateSeasonMatch(animeSeason, preferredSeasons) {
    if (!animeSeason || !preferredSeasons.length)
        return 0;
 ...
 */
export function generateRecommendations(availableAnimes, preferences, userRatings, userFavorites, limit = 10) {
    ...
    for (const anime of availableAnimes) {
        ...
        if (excludedIds.has(anime.id))
            continue;
        // Skip if score is too low
        if (anime.score && anime.score < 6.0)
            continue;
        ...
        const totalScore = (genreScore * RECOMMENDATION_WEIGHTS.genreMatch) +
 ...
 */
export function filterByRatingRange(animes, minRating, maxRating) {
 ...
 */
export function groupAnimesByGenre(animes) {
    const groups = {};
 ...
 */
export function calculateUserGenrePreferences(ratings) {
    const genreCounts = {};
